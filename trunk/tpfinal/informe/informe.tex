...
\begin{IEEEkeywords}
Visión Computacional, OpenCV, Mapas de Disparidad, calibración estéreo, geometría epipolar, reproyección, región de interes, visión robótica, navegación.
\end{IEEEkeywords}}
...
La heurística consiste en recorrer la región de interes calculando la mediana de los valores de disparidad de cada columna. Es decir, aplanamos el ROI\footnote{Region Of Interest} a una sola fila que contiene en cada celda el valor de disparidad medio de la columna (ver figura). Luego, calculamos la mediana de esta fila consiguiendo el valor de disparidad medio del ROI.
...
\begin{itemize}
	...
	\item Obstaculos grandes.
	\item Obstaculos separados por una distancia considerable.
	\item Los obstaculos están apoyados en la superficie por la que se mueve el robot. Es decir, no hay objetos colgados.
	\item Los obstaculos no deben moverse.
\end{itemize}
...
%La capacidad de procesamiento fue mas que suficiente para la ejecucion del mismo, entonces podemos afirmar que nuestro %algoritmo no posee altas restricciones de performance. 
	\item El robot, que posee una microcomputadora que corre un linux 2.6 portable. El mismo está conectado a la netbook por un cable de red (RJ-45).%El cual realiza el movimiento especificado.
...
Por otro lado, utilizamos la libreria Libcam que nos permite capturar imágenes de alta calidad aprovechando al máximo la cámara estéreo.
...
El mapa de disparidad es un método efieciente para almacenar la profundidad o distancia de cada pixel. Con esta información utilizamos...
...
$[1]$ Region of interest in disparity mapping for navigation of stereo vision autonomous guided vehicle.\\
